...
def runABC(objfun,NP, D, lb, ub, runtime, maxCycle, stopCost):
    ...
    limit = NP*2     #A food source which could not be improved through "limit" trials is abandoned by its employed bee
    ...
    GlobalParameters=np.zeros([runtime,D])
    for r in range(0,runtime):
        ...
        while((iters!=maxCycle) and (GlobalMin > stopCost)):
            for i in range(0,FoodNumber):
                ...
                sol[Param2Change]=Foods[i][Param2Change]+(Foods[i][Param2Change]-Foods[neighbour][Param2Change])*(np.random.rand()-0.5)*2
...
#%In Basic ABC, only one scout is allowed to occur in each cycle*/
            ...
            if (trial[0][ind]>limit):
                trial[0][ind]=0
                sol=(ub-lb)*np.random.rand(1,D)+lb
                ObjValSol=objfun(sol)
            ...
            print 'ïter='+str(iters)+" ObjVal="+str(GlobalMin)
            iters=iters+1
        #pdb.set_trace()
